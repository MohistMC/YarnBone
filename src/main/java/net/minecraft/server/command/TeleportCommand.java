...
public class TeleportCommand {
    ...
    private static int execute(ServerCommandSource source, Collection<? extends Entity> targets, ServerWorld world, PosArgument location, @Nullable PosArgument rotation, @Nullable LookTarget facingLocation) throws CommandSyntaxException {
        ...
        if (targets.size() == 1) {
            source.sendFeedback(Text.translatable("commands.teleport.success.location.single", targets.iterator().next().getDisplayName(), TeleportCommand.formatFloat(lv.x), TeleportCommand.formatFloat(lv.y), TeleportCommand.formatFloat(lv.z)), true);
        } else {
            source.sendFeedback(Text.translatable("commands.teleport.success.location.multiple", targets.size(), TeleportCommand.formatFloat(lv.x), TeleportCommand.formatFloat(lv.y), TeleportCommand.formatFloat(lv.z)), true);
        }
        return targets.size();
    }
