...
public class CommandManager {
    ...
    public int execute(ParseResults<ServerCommandSource> parseResults, String command) {
        ...
        try {
        ...
        catch (CommandException lv2) {
            lv.sendError(lv2.getTextMessage());
            int n = 0;
            return n;
        }
        catch (CommandSyntaxException commandSyntaxException) {
            int i;
            lv.sendError(Texts.toText(commandSyntaxException.getRawMessage()));
            if (commandSyntaxException.getInput() != null && commandSyntaxException.getCursor() >= 0) {
                i = Math.min(commandSyntaxException.getInput().length(), commandSyntaxException.getCursor());
                MutableText lv3 = Text.empty().formatted(Formatting.GRAY).styled(style -> style.withClickEvent(new ClickEvent(ClickEvent.Action.SUGGEST_COMMAND, "/" + command)));
                if (i > 10) {
                    lv3.append(ScreenTexts.ELLIPSIS);
                }
                lv3.append(commandSyntaxException.getInput().substring(Math.max(0, i - 10), i));
                if (i < commandSyntaxException.getInput().length()) {
                    MutableText lv4 = Text.literal(commandSyntaxException.getInput().substring(i)).formatted(Formatting.RED, Formatting.UNDERLINE);
                    lv3.append(lv4);
                }
                lv3.append(Text.translatable("command.context.here").formatted(Formatting.RED, Formatting.ITALIC));
                lv.sendError(lv3);
            }
            i = 0;
            return i;
            ...
            MutableText lv5 = Text.literal(exception.getMessage() == null ? exception.getClass().getName() : exception.getMessage());
            ...
            lv.sendError(Text.translatable("command.failed").styled(style -> style.withHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, lv5))));
            if (SharedConstants.isDevelopment) {
    ...
    private void makeTreeForSource(CommandNode<ServerCommandSource> tree, CommandNode<CommandSource> result, ServerCommandSource source2, Map<CommandNode<ServerCommandSource>, CommandNode<CommandSource>> resultNodes) {
        for (CommandNode<ServerCommandSource> commandNode3 : tree.getChildren()) {
            ...
            if (!commandNode3.canUse(source2)) continue;
            ...
            this.makeTreeForSource(commandNode3, commandNode4, source2, resultNodes);
        }
    }
    ...
    public static CommandRegistryAccess createRegistryAccess(final RegistryWrapper.WrapperLookup registryLookup) {
        return new CommandRegistryAccess(){
            ...
            public <T> RegistryWrapper<T> createWrapper(RegistryKey<? extends Registry<T>> registryRef) {
                ...
                return new RegistryWrapper.Delegating<T>(lv){
                    ...
                    public RegistryEntryList.Named<T> getOrThrow(TagKey<T> tag) {
                        Optional<RegistryEntryList.Named<RegistryEntryList.Named>> optional = lv.getOptional(tag);
                        return optional.orElseGet(() -> RegistryEntryList.of(lv, tag));
                    }
                };
            }
        };
