...
public class EffectCommand {
    ...
    private static int executeGive(ServerCommandSource source, Collection<? extends Entity> targets, RegistryEntry<StatusEffect> statusEffect, @Nullable Integer seconds, int amplifier, boolean showParticles) throws CommandSyntaxException {
        ...
        for (Entity entity : targets) {
            ...
            if (!(entity instanceof LivingEntity) || !((LivingEntity)entity).addStatusEffect(lv3 = new StatusEffectInstance(lv, k, amplifier, false, showParticles), source.getEntity())) continue;
            ++j;
        }
        if (j == 0) {
        ...
        if (targets.size() == 1) {
            source.sendFeedback(Text.translatable("commands.effect.give.success.single", lv.getName(), targets.iterator().next().getDisplayName(), k / 20), true);
        } else {
    ...
    private static int executeClear(ServerCommandSource source, Collection<? extends Entity> targets, RegistryEntry<StatusEffect> statusEffect) throws CommandSyntaxException {
        ...
        int i = 0;
        for (Entity entity : targets) {
            if (!(entity instanceof LivingEntity) || !((LivingEntity)entity).removeStatusEffect(lv)) continue;
            ++i;
        }
        if (i == 0) {
        ...
        if (targets.size() == 1) {
            source.sendFeedback(Text.translatable("commands.effect.clear.specific.success.single", lv.getName(), targets.iterator().next().getDisplayName()), true);
        } else {
