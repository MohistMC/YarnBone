...
extends ShiftableStructurePiece {
    ...
    private void generateBasementStairs(BlockPos pos, StructureWorldAccess world, BlockBox chunkBox) {
        ...
        this.addBlock(world, lv.rotate(BlockRotation.COUNTERCLOCKWISE_90), 12, 0, 17, chunkBox);
        ...
        this.addBlock(world, lv2, i - 1, j + 3, k + 4, chunkBox);
        this.addBlock(world, lv2, i, j + 3, k + 4, chunkBox);
        this.addBlock(world, lv2, i - 1, j + 2, k + 4, chunkBox);
        this.addBlock(world, bl ? lv2 : lv3, i, j + 2, k + 4, chunkBox);
        this.addBlock(world, bl ? lv2 : lv3, i, j + 1, k + 4, chunkBox);
    }
        ...
        for (int o = startY; o <= endY; ++o) {
            for (int p = startX; p <= endX; ++p) {
                for (int q = startZ; q <= endZ; ++q) {
                    this.addPotentialSuspiciousSandPosition(p, o, q, chunkBox);
                }
            }
        }
    }
