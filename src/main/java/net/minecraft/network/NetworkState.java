...
public enum NetworkState implements PacketBundleHandler.BundlerGetter
{
    ...
    public Int2ObjectMap<Class<? extends Packet<?>>> getPacketIdToPacketMap(NetworkSide side) {
        ...
        PacketHandler<?> lv = this.packetHandlers.get((Object)side);
        if (lv == null) {
            return Int2ObjectMaps.emptyMap();
        }
        lv.packetIds.forEach((packetId, integer) -> int2ObjectMap.put((int)integer, (Class<? extends Packet<?>>)packetId));
    ...
    static {
        ...
        for (NetworkState lv : NetworkState.values()) {
            ...
            lv.packetHandlers.forEach((side, handler) -> handler.forEachPacketType(packetClass -> {
                ...
                HANDLER_STATE_MAP.put((Class<Packet<?>>)packetClass, lv);
            }));
        }
    }
    ...
    static class PacketHandler<T extends PacketListener> {
        ...
        public <P extends BundlePacket<T>> PacketHandler<T> registerBundlePacket(Class<P> bundlePacketType, Function<Iterable<Packet<T>>, P> bundleFunction) {
            ...
            this.register(BundleSplitterPacket.class, buf -> lv);
