...
public class PiglinBrain {
    ...
    private static List<ItemStack> getBarteredItem(PiglinEntity piglin) {
        ...
        ObjectArrayList<ItemStack> list = lv.generateLoot(new LootContext.Builder((ServerWorld)piglin.world).parameter(LootContextParameters.THIS_ENTITY, piglin).random(piglin.world.random).build(LootContextTypes.BARTER));
        return list;
    }
    ...
    private static Task<LivingEntity> makeRememberRideableHoglinTask() {
        ...
        return MemoryTransferTask.create(entity -> entity.isBaby() && lv.shouldRun(entity.world.random), MemoryModuleType.NEAREST_VISIBLE_BABY_HOGLIN, MemoryModuleType.RIDE_TARGET, RIDE_TARGET_MEMORY_DURATION);
    }
