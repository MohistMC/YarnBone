...
implements InventoryOwner {
    ...
    public AllayEntity(EntityType<? extends AllayEntity> arg, World arg2) {
        ...
        EntityPositionSource lv = new EntityPositionSource(this, this.getStandingEyeHeight());
        this.listenerCallback = new VibrationListenerCallback();
        this.gameEventHandler = new EntityGameEventHandler<VibrationListener>(new VibrationListener(lv, 16, this.listenerCallback));
        this.jukeboxEventHandler = new EntityGameEventHandler<JukeboxEventListener>(new JukeboxEventListener(lv, GameEvent.JUKEBOX_PLAY.getRange()));
    }
    ...
    private boolean areDifferentPotions(ItemStack stack, ItemStack stack2) {
        ...
        boolean bl3 = bl = lv != null && lv.contains("Potion");
        if (!bl) {
            return false;
        }
        NbtCompound lv2 = stack2.getNbt();
        boolean bl4 = bl2 = lv2 != null && lv2.contains("Potion");
        if (!bl2) {
            return true;
        }
    ...
    public void writeCustomDataToNbt(NbtCompound nbt) {
        ...
        VibrationListener.createCodec(this.listenerCallback).encodeStart(NbtOps.INSTANCE, this.gameEventHandler.getListener()).resultOrPartial(LOGGER::error).ifPresent(arg2 -> nbt.put("listener", (NbtElement)arg2));
    ...
    public void readCustomDataFromNbt(NbtCompound nbt) {
        ...
        if (nbt.contains("listener", NbtElement.COMPOUND_TYPE)) {
            VibrationListener.createCodec(this.listenerCallback).parse(new Dynamic<NbtCompound>(NbtOps.INSTANCE, nbt.getCompound("listener"))).resultOrPartial(LOGGER::error).ifPresent(arg -> this.gameEventHandler.setListener((VibrationListener)arg, this.world));
        }
    ...
    private void duplicate() {
        AllayEntity lv = EntityType.ALLAY.create(this.world);
        if (lv != null) {
