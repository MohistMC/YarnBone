...
implements VariantHolder<FrogVariant> {
    ...
    public void readCustomDataFromNbt(NbtCompound nbt) {
        ...
        FrogVariant lv = Registries.FROG_VARIANT.get(Identifier.tryParse(nbt.getString(VARIANT_KEY)));
        if (lv != null) {
            this.setVariant(lv);
        }
    }
    ...
    public void breed(ServerWorld world, AnimalEntity other) {
        ServerPlayerEntity lv = this.getLovingPlayer();
        if (lv == null) {
            lv = other.getLovingPlayer();
        }
        if (lv != null) {
            lv.incrementStat(Stats.ANIMALS_BRED);
            Criteria.BRED_ANIMALS.trigger(lv, this, other, null);
        }
