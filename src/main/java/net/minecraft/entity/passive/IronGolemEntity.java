...
implements Angerable {
    ...
    public void tickMovement() {
        ...
        if (this.getVelocity().horizontalLengthSquared() > 2.500000277905201E-7 && this.random.nextInt(5) == 0 && !(lv = this.world.getBlockState(new BlockPos(i = MathHelper.floor(this.getX()), j = MathHelper.floor(this.getY() - (double)0.2f), k = MathHelper.floor(this.getZ())))).isAir()) {
    ...
    public boolean tryAttack(Entity target) {
        ...
        if (bl) {
            ...
            if (target instanceof LivingEntity) {
                ...
                d = lv.getAttributeValue(EntityAttributes.GENERIC_KNOCKBACK_RESISTANCE);
            } else {
                d = 0.0;
            }
            double d2 = d;
    ...
    public static enum Crack {
        ...
        static {
            VALUES = Stream.of(Crack.values()).sorted(Comparator.comparingDouble(arg -> arg.maxHealthFraction)).collect(ImmutableList.toImmutableList());
        }
    }
}
