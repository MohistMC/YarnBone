...
implements Shearable {
    ...
    private DyeColor getChildColor(AnimalEntity firstParent, AnimalEntity secondParent) {
        ...
        return this.world.getRecipeManager().getFirstMatch(RecipeType.CRAFTING, lv3, this.world).map(recipe -> recipe.craft(lv3, this.world.getRegistryManager())).map(ItemStack::getItem).filter(DyeItem.class::isInstance).map(DyeItem.class::cast).map(DyeItem::getColor).orElseGet(() -> this.world.random.nextBoolean() ? lv : lv2);
    }
