...
public class Structures {
    ...
    public static void bootstrap(Registerable<Structure> structureRegisterable) {
        ...
        structureRegisterable.register(StructureKeys.ANCIENT_CITY, new JigsawStructure(Structures.createConfig(lv.getOrThrow(BiomeTags.ANCIENT_CITY_HAS_STRUCTURE), Arrays.stream(SpawnGroup.values()).collect(Collectors.toMap(arg -> arg, arg -> new StructureSpawns(StructureSpawns.BoundingBox.STRUCTURE, Pool.empty()))), GenerationStep.Feature.UNDERGROUND_DECORATION, StructureTerrainAdaptation.BEARD_BOX), lv2.getOrThrow(AncientCityGenerator.CITY_CENTER), Optional.of(new Identifier("city_anchor")), 7, ConstantHeightProvider.create(YOffset.fixed(-27)), false, Optional.empty(), 116));
    }
}
