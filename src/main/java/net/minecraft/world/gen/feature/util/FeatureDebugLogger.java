...
public class FeatureDebugLogger {
    ...
    public static void dump() {
        ...
        FEATURES.asMap().forEach((world, features) -> {
            ...
            features.featureData().forEach((featureData, count) -> LOGGER.debug(string2 + " " + String.format(Locale.ROOT, "%10d ", count) + String.format(Locale.ROOT, "%10f ", (double)count.intValue() / (double)integer.intValue()) + featureData.topFeature().flatMap(lv::getKey).map(RegistryKey::getValue) + " " + featureData.feature().feature() + " " + featureData.feature()));
        });
    }
