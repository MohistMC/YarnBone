...
public class MultiNoiseBiomeSourceParameterList {
    ...
    public record Preset(Identifier id, BiomeSourceFunction biomeSourceFunction) {
        ...
        static final Map<Identifier, Preset> BY_IDENTIFIER = Stream.of(NETHER, OVERWORLD, OVERWORLD_UPDATE_1_20).collect(Collectors.toMap(Preset::id, arg -> arg));
        public static final Codec<Preset> CODEC = Identifier.CODEC.flatXmap(arg -> Optional.ofNullable(BY_IDENTIFIER.get(arg)).map(DataResult::success).orElseGet(() -> DataResult.error(() -> "Unknown preset: " + arg)), arg -> DataResult.success(arg.id));
