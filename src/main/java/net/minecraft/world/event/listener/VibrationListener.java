...
implements GameEventListener {
        ...
        if (world instanceof ServerWorld) {
            ...
            if (this.vibration == null) {
                this.selector.getVibrationToTick(lv.getTime()).ifPresent(vibration -> {
                    this.vibration = vibration;
                    Vec3d lv = this.vibration.pos();
                    this.delay = MathHelper.floor(this.vibration.distance());
                    lv.spawnParticles(new VibrationParticleEffect(this.positionSource, this.delay), lv.x, lv.y, lv.z, 1, 0.0, 0.0, 0.0, 0.0);
            ...
            if (this.vibration != null) {
                ...
                if (this.delay <= 0) {
                    this.delay = 0;
                    this.callback.accept(lv, this, BlockPos.ofFloored(this.vibration.pos()), this.vibration.gameEvent(), this.vibration.getEntity(lv).orElse(null), this.vibration.getOwner(lv).orElse(null), this.vibration.distance());
                    this.vibration = null;
                }
            }
        }
    ...
    public boolean listen(ServerWorld world, GameEvent event, GameEvent.Emitter emitter, Vec3d emitterPos) {
        ...
        if (VibrationListener.isOccluded(world, emitterPos, lv)) {
            return false;
        }
