...
implements Comparable<LevelSummary> {
    ...
    private Text createDetails() {
        ...
        MutableText mutableText = lv = this.isHardcore() ? Text.empty().append(Text.translatable("gameMode.hardcore").styled(style -> style.withColor(-65536))) : Text.translatable("gameMode." + this.getGameMode().getName());
        if (this.hasCheats()) {
            lv.append(", ").append(Text.translatable("selectWorld.cheats"));
        }
