...
public class BrewingRecipeRegistry {
    ...
    private static final Predicate<ItemStack> POTION_TYPE_PREDICATE = stack -> {
        for (Ingredient lv : POTION_TYPES) {
            if (!lv.test((ItemStack)stack)) continue;
            return true;
        }
        return false;
    };
    ...
    protected static boolean hasItemRecipe(ItemStack input, ItemStack ingredient) {
        ...
        for (int i = 0; i < j; ++i) {
            ...
            if (lv2.input != lv || !lv2.ingredient.test(ingredient)) continue;
            return true;
        }
        return false;
    }
    ...
    public static ItemStack craft(ItemStack ingredient, ItemStack input) {
        if (!input.isEmpty()) {
            ...
            int j = ITEM_RECIPES.size();
            for (i = 0; i < j; ++i) {
                lv3 = ITEM_RECIPES.get(i);
                if (lv3.input != lv2 || !lv3.ingredient.test(ingredient)) continue;
                return PotionUtil.setPotion(new ItemStack((ItemConvertible)lv3.output), lv);
            }
            j = POTION_RECIPES.size();
            for (i = 0; i < j; ++i) {
                lv3 = POTION_RECIPES.get(i);
                if (lv3.input != lv || !lv3.ingredient.test(ingredient)) continue;
