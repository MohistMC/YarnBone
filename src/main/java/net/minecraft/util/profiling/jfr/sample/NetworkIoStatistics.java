    ...
    public record Packet(NetworkSide side, int protocolId, int packetId) {
        ...
        static {
            ...
            for (NetworkState lv : NetworkState.values()) {
                for (NetworkSide lv2 : NetworkSide.values()) {
                    Int2ObjectMap<Class<net.minecraft.network.packet.Packet<?>>> int2ObjectMap = lv.getPacketIdToPacketMap(lv2);
                    int2ObjectMap.forEach((BiConsumer<Integer, Class<net.minecraft.network.packet.Packet<?>>>)((BiConsumer<Integer, Class>)(packetId, clazz) -> builder.put(new Packet(lv2, lv.getId(), (int)packetId), clazz.getSimpleName())));
                }
            }
