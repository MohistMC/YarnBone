...
extends Fluid {
    ...
    public Vec3d getVelocity(BlockView world, BlockPos pos, FluidState state) {
        ...
        for (Direction lv2 : Direction.Type.HORIZONTAL) {
            ...
            if (f == 0.0f) {
                Vec3i lv4;
                FluidState lv5;
                if (!world.getBlockState(lv).getMaterial().blocksMovement() && this.isEmptyOrThis(lv5 = world.getFluidState((BlockPos)(lv4 = lv.down()))) && (f = lv5.getHeight()) > 0.0f) {
    ...
    protected int getFlowSpeedBetween(WorldView world, BlockPos arg2, int i, Direction arg3, BlockState arg4, BlockPos arg5, Short2ObjectMap<Pair<BlockState, FluidState>> short2ObjectMap, Short2BooleanMap short2BooleanMap) {
        int j = 1000;
        for (Direction lv : Direction.Type.HORIZONTAL) {
            int k;
            if (lv == arg3) continue;
            BlockPos lv2 = arg2.offset(lv);
            short s2 = FlowableFluid.method_15747(arg5, lv2);
            ...
            if (i >= this.getFlowSpeed(world) || (k = this.getFlowSpeedBetween(world, lv2, i + 1, lv.getOpposite(), lv3, arg5, short2ObjectMap, short2BooleanMap)) >= j) continue;
            j = k;
        }
        return j;
    }
    ...
    protected Map<Direction, FluidState> getSpread(World world, BlockPos pos, BlockState state) {
        ...
        for (Direction lv : Direction.Type.HORIZONTAL) {
            BlockPos lv2 = pos.offset(lv);
            short s2 = FlowableFluid.method_15747(pos, lv2);
            Pair pair = short2ObjectMap.computeIfAbsent(s2, s -> {
                BlockState lv = world.getBlockState(lv2);
                return Pair.of(lv, lv.getFluidState());
            });
