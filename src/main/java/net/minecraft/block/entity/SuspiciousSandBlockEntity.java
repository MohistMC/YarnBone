...
extends BlockEntity {
    ...
    public void generateItem(PlayerEntity player) {
        ...
        LootTable lv = this.world.getServer().getLootManager().getTable(this.lootTable);
        ...
        ObjectArrayList<ItemStack> objectArrayList = lv.generateLoot(lv3.build(LootContextTypes.CHEST));
    ...
    private void spawnItem(PlayerEntity player) {
        ...
        if (!this.item.isEmpty()) {
            ...
            double g = Math.floor(lv2.getX()) + 0.5 * e + f;
            double h = Math.floor((double)lv2.getY() + 0.5) + (double)(EntityType.ITEM.getHeight() / 2.0f);
            double i = Math.floor(lv2.getZ()) + 0.5 * e + f;
    ...
    public NbtCompound toInitialChunkDataNbt() {
        ...
        lv.put(ITEM_NBT_KEY, this.item.writeNbt(new NbtCompound()));
        return lv;
    }
