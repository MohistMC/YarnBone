...
extends WallMountedBlock {
    ...
    protected void tryPowerWithProjectiles(BlockState state, World world, BlockPos pos) {
        boolean bl2;
        PersistentProjectileEntity lv = this.wooden ? (PersistentProjectileEntity)world.getNonSpectatingEntities(PersistentProjectileEntity.class, state.getOutlineShape(world, pos).getBoundingBox().offset(pos)).stream().findFirst().orElse(null) : null;
        boolean bl = lv != null;
