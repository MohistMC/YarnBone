...
extends Block {
    ...
    private BlockState getStateWithInstrument(WorldAccess world, BlockPos pos, BlockState state) {
        if (NoteBlock.areMobHeadSoundsEnabled(world)) {
            ...
            return (BlockState)state.with(INSTRUMENT, Instrument.fromAboveState(lv).orElseGet(() -> Instrument.fromBelowState(world.getBlockState(pos.down()))));
        }
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
        ItemStack lv;
        if (NoteBlock.areMobHeadSoundsEnabled(world) && (lv = player.getStackInHand(hand)).isIn(ItemTags.NOTEBLOCK_TOP_INSTRUMENTS) && hit.getSide() == Direction.UP) {
    ...
    public boolean onSyncedBlockEvent(BlockState state, World world, BlockPos pos, int type, int data) {
        ...
        float f;
        Instrument lv = state.get(INSTRUMENT);
