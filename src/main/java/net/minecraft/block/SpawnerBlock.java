...
extends BlockWithEntity {
    ...
    private Optional<Text> getEntityNameForTooltip(ItemStack stack) {
        ...
        if (lv != null && lv.contains("SpawnData", NbtElement.COMPOUND_TYPE) && (lv2 = Identifier.tryParse(string = lv.getCompound("SpawnData").getCompound("entity").getString("id"))) != null) {
            return Registries.ENTITY_TYPE.getOrEmpty(lv2).map(entityType -> Text.translatable(entityType.getTranslationKey()).formatted(Formatting.GRAY));
        }
